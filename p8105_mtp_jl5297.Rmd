...
The range of activity counts is (`r range(activity_df$activity_counts)`) and the mean is `r round(mean(activity_df$activity_counts),2)`. The distribution of `activity_counts` is extremely skewed where some points are much larger than the bulk of the data. When the device detects no activity, activity counts equal to 1.
