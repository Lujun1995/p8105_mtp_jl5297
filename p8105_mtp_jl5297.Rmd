...
str(activity_df)
```
We load the data from  “p8105_mtp_data.csv” and use `gather` to go from wide to long formats. Two variables(`minute` and `day`) are transformed into integer and factor. There are `r nrow(activity_df)` observations and `r ncol(activity_df)` variables.
...
We can see the distribution of `activity_counts` is extremely skewed where some points are much larger than the bulk of the data. The range of activity counts is `r range(activity_df$activity_counts)`. When this participant has no activity, activity counts equal to 1.
...
```
We make a density plot of log(activity counts) by hour. We can see this participant tends to have activity from 7 a. m. to 10 p. m.

## 24-hour activity profiles analysis
```{r 24-hour, fig.width=8, fig.asp = 0.6}
activity_df %>% 
...
We divide weeks into 3 periods and aggregate data into 20-minute epochs. Then we make a plot which shows 24-hour activity profiles for each day in panels defined by period and day of the week. Smooth estimates of each period are emphasized as bold curves.
